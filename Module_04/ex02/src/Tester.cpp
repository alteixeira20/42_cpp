...
void	test_wrong_polymorphism(void)
{
	...
	std::cout << "\nCalling makeSound() through WrongAnimal pointer:" << std::endl;

	wa->makeSound();  // OK: WrongAnimal sound
	wc->makeSound();  // WRONG: still WrongAnimal sound (no virtual)

	std::cout << "\nDeleting WrongAnimals:" << std::endl;

	delete wa;        // WrongAnimal destructor ONLY
	delete wc;        // WrongAnimal destructor ONLY (WRONG behavior)

	std::cout << "\nExpected result:" << std::endl;
	std::cout << "- WrongCat sound is NOT heard" << std::endl;
	std::cout << "- WrongCat destructor is NOT called" << std::endl;
}
